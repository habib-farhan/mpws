...
{
    return [self findBestMatchCenter:NULL angle:NULL score:NULL mode: [self cRead] key:@"0"];
}
- (NSString *)findBestMatchCenter:(CGPoint*)outCenter angle:(float*)outRadians score:(float*)outScore mode:(int)mode key:(NSString*) key
{
	@autoreleasepool {
		...
		Translate(samples, samplePoints, -center.x, -center.y); // Recenter
		//-------------------------------------------------------------------------------------------------------------------------------------
        //If the widget is in edit mode and stores the gesture points in "templates" and in the Gestures.json
        if (mode == [self cAdd]) {
            NSMutableArray *temp = [NSMutableArray arrayWithCapacity:samplePoints];
            for (i = 0; i< samplePoints; i++)
            {
                    [temp insertObject: [NSValue valueWithCGPoint:samples[i]] atIndex:i];
            }
            [templates setObject: temp forKey: [NSString stringWithFormat:@"%@", key]];
		...
		NSLog(@"Read:\n%@", string);
#endif
        if(mode == [self cRemove]){
            if([templates count] > 0){
                [templates removeObjectForKey:bestTemplateName];
                [self saveTemplateData];
            }
        }
...
//Creates a string in JSON forat from templates and stores the in a file
-(void) saveTemplateData{
    ...
    for(NSString *key in templates){
        ...
        [string appendString:@"\": [ "];
        NSMutableArray *samples = [templates mutableArrayValueForKey:key];
        for (NSValue *point in samples){
            CGPoint pt;
            [point getValue:&pt];
            [string appendFormat:@"[%0.2f, %0.2f], ", pt.x, pt.y];
        }
    ...
    NSString *path = [documentsDirectory stringByAppendingPathComponent:@"Gestures.json"];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if ([fileManager fileExistsAtPath:path]){
        [string writeToFile:path atomically:YES encoding:NSUTF8StringEncoding error:nil];
    }else{
        NSLog(@"No gesture file found");
    }
}
